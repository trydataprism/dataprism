...
export async function deactivateVisitor(websiteId: string, sessionId: string): Promise<void> {
  await db
    .update(realTimeVisitors)
    .set({
      isActive: false,
...
export async function getActiveVisitors(websiteId: string): Promise<{ visitors: RealTimeVisitor[]; count: number }> {
...
export async function cleanupExpiredVisitors(websiteId: string): Promise<void> {
  await db
    .delete(realTimeVisitors)
    .where(
      and(
